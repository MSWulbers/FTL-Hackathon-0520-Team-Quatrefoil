...
```{r}
# Negative Wordcloud
corp_neg <- preprocess_corpus(data_most_negative$Text)
wordcloud(corp_neg, max.words = 15, colors = brewer.pal(9, "Paired"), rot.per = 0)
# title("Negative", sub = "Most frequently used words in negative 'face mask' comments")
```


```{r}
# Positive wordcloud
corp_pos <- preprocess_corpus(data_most_positive$Text)
wordcloud(corp_pos, max.words = 15, colors = brewer.pal(4, "Paired"), rot.per = 0)
```
...
```{r}
# Wordcloud March
march_corpus <- preprocess_corpus(data_march$Text)
wordcloud(march_corpus, max.words = 15, colors = brewer.pal(11, "BrBG"), , rot.per = 0)
```

```{r}
# Wordcloud April
wordcloud(preprocess_corpus(data_april$Text), max.words = 20, colors = brewer.pal(7, "Accent"))
```

```{r}
# Wordcloud May
may_corpus <- preprocess_corpus(data_may$Text)
wordcloud(may_corpus, max.words = 15, colors = brewer.pal(11, "PuOr"))
```
